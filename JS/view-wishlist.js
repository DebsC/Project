...
const generateWishlistDOM = () => {
    const myWishlist = document.getElementById("wishlist-content")
    const WISHlist = getWishlist()

     let htmlWL = ''

     if (WISHlist.length === 0) {
         htmlWL = '<h3>No concerts saved in the wishlist</h3>'
     } else {
        WISHlist.forEach((concert) => {
             htmlWL += renderWishlistElement(concert)
         })
     }
     myWishlist.innerHTML = htmlWL
 }
